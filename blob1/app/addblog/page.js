...
function addBlog() {
 ...
 let [data,setData]=useState([]);
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:8080/user/1/allblogs").then(
    (response) => {
    ...
    let buttonHandler=()=>{
        axios.post("http://localhost:8080/user/1/addblog", {
    ...
    const handleContentChange = (e) => {
        ...
        if(e.target.getAttribute("name") === "heading" || e.target.getAttribute("name") === "description") {
            ...
            idx = parseInt(idx, 10); // Convert to integer
               ...
               >{allDetails.heading1?allDetails.heading1:"loading" }</h1>
                    ...
                    suppressContentEditableWarning={true}
                        ...
                        >
                           ...
                           <div className=" hover:border-2 relative hover:border-dotted hover:border-gray-500 my-4">
                           
                           <p className="absolute top-[-2] right-0 border p-1 rounded-md bg-white" onClick={()=>{
                        let newSection = {
                               heading: "New Section",
                               description: "This is the content of the new section that you can edit and add more details to it."
                           };
                           setAllDetails(prevDetails => ({ ...prevDetails, p1: [...prevDetails.p1, newSection] }));
                           console.log(allDetails);
                           }}>+ Add Section</p>
                           </div>
                    {/* <div>
